...
You see that this controller implements the previous `behavior` specification in the `get` and `post` functions.
Even though if `post` is not allowed and returns error 400.
...
The views `render()` function takes a model (a map) where we here just specify some `:reader_input` definitions. Those are later rendered as a table with label and textfield.
...
So we already have the model in the play here that is used by both controller and view.


##### A view controller with submit

To support a submit you certainly have to implement the `post` function. The `post` function in the controller will receive the form parameters as a list of tuples.  
...
When you got the form parameters you just have to transform the data to suite your needs in order to process it further.  
OK, there might be a bit of manual parsing required. But we're developers, right?
